...
namespace HaiKanTravelManagementSystem.Api.Controllers.Api.V1.Guide
{
    ...
    [ApiController]
    [CustomAuthorize]
    public class CulturalIntroducController : ControllerBase
    {
        ...
        public CulturalIntroducController(haikanHeQiaoContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List(CulturalRequestPayload payload)
        {
            using (_dbContext)
            {
                var query = _dbContext.CulturalIntroduc.Where(x=>x.IsDeleted==0).Select(x => new
                {
                    x.CulturalIntroducUuid,
                    x.Title,
                    x.IsDeleted,
                    x.Id,
                ...
                ToLog.AddLog("查询", "成功:查询:人文介绍列表数据", _dbContext);
                return Ok(response);
            }
        }
        ...
        public IActionResult Load(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.CulturalIntroduc.FirstOrDefault(x => x.CulturalIntroducUuid == guid);
                var response = ResponseModelFactory.CreateInstance;
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE CulturalIntroduc SET IsDeleted=@IsDel WHERE CulturalIntroducUUID IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@IsDel", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:人文介绍列表数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
