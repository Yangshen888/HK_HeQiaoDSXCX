...
namespace HaiKanTravelManagementSystem.Api.Controllers.Api.V1.Guide
{
    ...
    [ApiController]
    [CustomAuthorize]
    public class PublicityPictureController : ControllerBase
    {
        ...
        public PublicityPictureController(haikanHeQiaoContext dbContext, IMapper mapper)
        {
        ...
        [HttpPost]
        public IActionResult List(PublicityPRequestPayload payload)
        {
            using (_dbContext)
            {
                var query = _dbContext.PublicityPicture.Where(x=>x.IsDeleted==0).Select(x => new
                {
                    x.PublicityPictureUuid,
                    x.Title,
                    x.IsDeleted,
                    x.Id,
                ...
                ToLog.AddLog("查询", "成功:查询:宣传图片列表数据", _dbContext);
                return Ok(response);
            }
        }
        ...
        public IActionResult Load(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.PublicityPicture.FirstOrDefault(x => x.PublicityPictureUuid == guid);
                var response = ResponseModelFactory.CreateInstance;
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE PublicityPicture SET IsDeleted=@IsDel WHERE PublicityPictureUUID IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@IsDel", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("编辑", "成功:编辑:宣传图片列表数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
