...
namespace HaiKanTravelManagementSystem.Api.Controllers.Api.V1.SpotPublicity
{
    ...
    [ApiController]
    [CustomAuthorize]
    public class PublicityTypeController : ControllerBase
    {
        ...
        public PublicityTypeController(haikanHeQiaoContext dbContext, IMapper mapper)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(PublicityTypeRequestPayload payload)
        {
            using (_dbContext)
            {
                ...
                ToLog.AddLog("查询", "成功:查询:宣传类别管理列表数据", _dbContext);
                return Ok(response);
            }
        }
        #endregion
        ...
        public IActionResult Create(PublicityTypeViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;

            using (_dbContext)
            {
                if (_dbContext.PublicityType.Count(x => x.Name == model.Name) > 0)
                {
                    response.SetFailed("标识名称已存在");
                    return Ok(response);
                }

                var entity = _mapper.Map<PublicityTypeViewModel, PublicityType>(model);
                ...
                if (res > 0)
                {
                    ToLog.AddLog("添加", "成功:添加:宣传类别管理列表数据", _dbContext);
                }
                response.SetSuccess();
                return Ok(response);
            }
        ...
        public IActionResult Edit(Guid guid)
        {
            using (_dbContext)
            {
                ...
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<PublicityType, PublicityTypeViewModel>(entity));
                return Ok(response);
            }
        }
        /// <summary>
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(PublicityTypeViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;

            using (_dbContext)
            {
                var entity = _dbContext.PublicityType.FirstOrDefault(x => x.PublicityTypeUuid == model.PublicityTypeUuid);
                if (entity == null)
                {
                    response.SetFailed("不存在");
                    return Ok(response);
                }
                if (_dbContext.PublicityType.Count(x => x.Name == model.Name && x.PublicityTypeUuid != model.PublicityTypeUuid) > 0)
                {
                ...
                if (res > 0)
                {
                    ToLog.AddLog("编辑", "成功:编辑:宣传类别管理列表数据", _dbContext);
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE PublicityType SET IsDeleted=@IsDel WHERE PublicityTypeUUID IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@IsDel", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:宣传类别管理列表数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
