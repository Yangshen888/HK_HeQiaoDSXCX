...
namespace HaiKanTravelManagementSystem.Api.Controllers.Api.V1.Walk
{
    ...
    public class SharingController : ControllerBase
    {
        ...
        public SharingController(haikanHeQiaoContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List(FoodRequestPayload payload)
        {
            using (_dbContext)
            {
                var query = from p in _dbContext.Sharing
                ...
                ToLog.AddLog("查询", "成功:查询:共享单车点位管理列表数据", _dbContext);
                return Ok(response);
            }
        }
        #endregion
        ...
        public IActionResult Edit(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Sharing.FirstOrDefault(x => x.SharingUuid == guid);
                var response = ResponseModelFactory.CreateInstance;
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE Sharing SET IsDelete=@IsDel WHERE SharingUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@IsDel", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:共享单车点位管理列表数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
