<template>
  <div>
    <Card>
      <dz-table
        :totalCount="stores.specialpick.query.totalCount"
        :pageSize="stores.specialpick.query.pageSize"
        :currentPage="stores.specialpick.query.currentPage"
      ...
      >
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.specialpick.query.kw"
                      placeholder="输入路线名称搜索..."
                      @on-search="handleSearchSpecialPick()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.specialpick.query.isDeleted"
                        @on-change="handleSearchSpecialPick"
                        placeholder="删除状态"
                        style="width: 60px"
                      >
                        <Option
                          v-for="item in stores.specialpick.sources
                            .isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                          >{{ item.text }}</Option
                      ...
                      <Select
                        slot="prepend"
                        v-model="stores.specialpick.query.state"
                        @on-change="handleSearchSpecialPick"
                        placeholder="发布状态"
                        style="width: 60px"
                      >
        ...
        <Table
          ...
          size="small"
          :highlight-row="true"
          :data="stores.specialpick.data"
          :columns="stores.specialpick.columns"
...
export default {
      ...
      stores: {
        specialpick: {
          ...
          columns: [
            ...
            { title: "路线名称", key: "name", tooltip: true, ellipsis: true },
            { title: "所属分类", key: "playType" },
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.luxianUuid);
    },
  },
  methods: {
    ...
    loadSpecialPickList() {
      getSpecialPickList(this.stores.specialpick.query).then((res) => {
        this.stores.specialpick.data = res.data.data;
        this.stores.specialpick.query.totalCount = res.data.totalCount;
        //console.log(this.stores.educaplan.data);
      });
    },
    handleSearchSpecialPick() {
      this.stores.specialpick.query.currentPage = 1;
      this.loadSpecialPickList();
    },
    handleRefresh() {
      this.stores.specialpick.query.currentPage = 1;
      this.loadSpecialPickList();
    ...
    handleSubmitPlan() {
      ...
      if (valid) {
        ...
        if (this.uploadList.length < 1) {
          this.$Message.warning("内容图片最少上传一张图片");
          return;
        }
        if (this.formModel.mode === "create") {
          this.docreateSpecialPick();
        }
        if (this.formModel.mode === "edit") {
          this.doEditPlan();
        }
    ...
    docreateSpecialPick() {
      createSpecialPick(this.formModel.fields).then((res) => {
        if (res.data.code === 200) {
    ...
    doEditPlan() {
      editSpecialPick(this.formModel.fields).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.handleCloseFormWindow();
          this.loadSpecialPickList();
    ...
    doDelete(ids) {
      ...
      deleteSpecialPick(ids).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.loadSpecialPickList();
          this.formModel.selection = [];
    ...
    handleEdit(row) {
      ...
      this.doLoadSpecialPick(row.luxianUuid);
    },
    handleShow(row) {
      this.handleResetFormRole();
    ...
    async doLoadSpecialPick(guid) {
      loadSpecialPick({ guid: guid }).then((res) => {
        ...
        if (res.data.data.cover != null && res.data.data.cover != "") {
          ...
          for (let i = 0; i < list.length; i++) {
            this.coveruploadList.push({
              url: config.baseUrl.dev + "UploadFiles/SpecialPick/" + list[i],
              status: "finished",
              name: "UploadFiles/SpecialPick/" + list[i],
              fileName: list[i],
            });
          }
        }
    ...
    handlePageChanged(page) {
      this.stores.specialpick.query.currentPage = page;
      this.loadSpecialPickList();
    },
    handlePageSizeChanged(pageSize) {
      this.stores.specialpick.query.pageSize = pageSize;
      this.loadSpecialPickList();
    },
  },
  mounted() {
