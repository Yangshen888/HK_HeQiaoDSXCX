<template>
  <div>
    <Card>
      <dz-table
        :totalCount="stores.publicitytype.query.totalCount"
        :pageSize="stores.publicitytype.query.pageSize"
        :currentPage="stores.publicitytype.query.currentPage"
      ...
      >
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.publicitytype.query.kw"
                      placeholder="输入名称搜索..."
                      @on-search="handleSearchPublicitytype()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.publicitytype.query.isDeleted"
                        @on-change="handleSearchPublicitytype"
                        placeholder="删除状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.publicitytype.sources.isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
        ...
        <Table
          ...
          size="small"
          :highlight-row="true"
          :data="stores.publicitytype.data"
          :columns="stores.publicitytype.columns"
...
<script>
  ...
  import {
    ...
    deletePublicitytype,
    batchCommand
  } from "@/api/spotpublicity/publicitytype";
  export default {
    name: "publicitytype",
    components: {
      Tables,
    ...
    data() {
      return {
        ...
        formModel: {
          ...
          rules: {
            name: [{type: "string",required: true,message: "请输入类别名称",trigger:'blur'}],
    ...
    methods: {
      loadPublicitytypeList() {
        getPublicitytypeList(this.stores.publicitytype.query).then(res => {
          this.stores.publicitytype.data = res.data.data;
          this.stores.publicitytype.query.totalCount = res.data.totalCount;
          //console.log(this.stores.educaplan.data);
        });
      },
      handleSearchPublicitytype() {
        this.stores.publicitytype.query.currentPage = 1;
      ...
      doDelete(ids) {
        ...
        deletePublicitytype(ids).then(res => {
          if (res.data.code === 200) {
            this.$Message.success(res.data.message);
            this.loadPublicitytypeList();
            this.formModel.selection = [];
      ...
      handleEdit(row) {
        ...
        this.doLoadPublicitytype(row.publicityTypeUuid);
      },
      ...
      handlePageChanged(page) {
        this.stores.publicitytype.query.currentPage = page;
        this.loadPublicitytypeList();
      },
      handlePageSizeChanged(pageSize) {
        this.stores.publicitytype.query.pageSize = pageSize;
        this.loadPublicitytypeList();
      }
    },
    mounted() {
