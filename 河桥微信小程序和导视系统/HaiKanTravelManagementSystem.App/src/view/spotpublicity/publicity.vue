<template>
  <div>
    <Card>
      <dz-table
        :totalCount="stores.publicity.query.totalCount"
        :pageSize="stores.publicity.query.pageSize"
        :currentPage="stores.publicity.query.currentPage"
      ...
      >
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.publicity.query.kw"
                      placeholder="输入名称搜索..."
                      @on-search="handleSearchPublicity()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.publicity.query.isDeleted"
                        @on-change="handleSearchPublicity"
                        placeholder="删除状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.publicity.sources.isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
                      <Select
                        slot="prepend"
                        v-model="stores.publicity.query.state"
                        @on-change="handleSearchPublicity"
                        placeholder="发布状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.publicity.sources.stateSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
                      <Select
                        slot="prepend"
                        v-model="stores.publicity.query.publicityTypeUuid"
                        @on-change="handleSearchPublicity"
                        placeholder="宣传类型"
                        style="width: 100px"
                      >
        ...
        <Table
          ...
          size="small"
          :highlight-row="true"
          :data="stores.publicity.data"
          :columns="stores.publicity.columns"
    ...
    >
      ...
      >
        ...
        <FormItem label="内容" label-position="top" prop="introduce">
          ...
          <quill-editor v-model="formModel.fields.introduce" >
          </quill-editor>
        ...
        <fieldset>
          ...
          <Row style="padding: 15px">
            ...
            <Upload
              ...
              :format="['jpg','jpeg','png']"
              :max-size="5120"
              :data="{fileSavePath:'Spotpublicity/Picture'}"
...
<script>
  ...
  import {
    ...
    deletetoFile,
    Publicitytypelist
  } from "@/api/spotpublicity/publicity";
  ...
  export default {
      ...
      return {
        ...
        formModel: {
          ...
          rules: {
            ...
            introduce: [{type: "string",required: true,message: "请输入内容",trigger:'blur'}],
            publicityTypeUuid: [{type: "string",required: true,message: "请选择类型",trigger:'blur'}],
          }
        },
        stores: {
          publicity: {
            ...
            columns: [
              ...
              { title: "状态", key: "state", align: "center", tooltip:true,ellipsis: true,minWidth: 70,slot:"state" },
              {
                title: "操作",
                fixed: "right",
                align: "center",
    ...
    computed: {
      ...
      selectedRowsId() {
        return this.formModel.selection.map(x => x.publicityUuid);
      }
    },
    methods: {
      ...
      loadPublicityList() {
        getPublicityList(this.stores.publicity.query).then(res => {
          this.stores.publicity.data = res.data.data;
          this.stores.publicity.query.totalCount = res.data.totalCount;
          //console.log(this.stores.educaplan.data);
        });
      },
      handleSearchPublicity() {
        this.stores.publicity.query.currentPage = 1;
        this.loadPublicityList();
      },
      handleRefresh() {
        this.stores.publicity.query.currentPage = 1;
        this.loadPublicityList();
      ...
      docreatePublicity() {
        createPublicity(this.formModel.fields).then(res => {
          if (res.data.code === 200) {
      ...
      doEditPlan() {
        editPublicity(this.formModel.fields).then(res => {
          if (res.data.code === 200) {
            this.$Message.success(res.data.message);
            this.handleCloseFormWindow();
            this.loadPublicityList();
      ...
      //单条删除
      handleDelete(row) {
        this.doDelete(row.publicityUuid);
      },
      doDelete(ids) {
        if (!ids) {
          this.$Message.warning("请选择至少一条数据");
      ...
      handleEdit(row) {
        ...
        this.doLoadPublicity(row.publicityUuid);
      },
      handleShow(row)
      {
      ...
      doLoadPublicity(guid) {
        loadPublicity({ guid: guid }).then(res => {
          ...
          if (res.data.data.picture != null) {
            ...
            for (let i = 0; i < list.length; i++) {
              this.uploadList.push({
                url:
                  config.baseUrl.dev + "UploadFiles/Spotpublicity/Picture/" + list[i],
                status: "finished",
                name: "UploadFiles/Spotpublicity/Picture/" + list[i],
                fileName: list[i]
              });
            }
          }
      ...
      handlePageChanged(page) {
        this.stores.publicity.query.currentPage = page;
        this.loadPublicityList();
      },
      handlePageSizeChanged(pageSize) {
        this.stores.publicity.query.pageSize = pageSize;
        this.loadPublicityList();
      },
