    ...
    >
      ...
      >
        ...
        <FormItem label="内容" label-position="top" prop="content">
          <quill-editor v-model="formModel.fields.content"></quill-editor>
        ...
        <fieldset>
          ...
          <Row style="padding: 15px">
            ...
            <Upload
              ...
              :format="['jpg','jpeg','png']"
              :max-size="5120"
              :data="{fileSavePath:'Guide/NaturalR'}"
    ...
    >
      ...
      >
        ...
        <fieldset>
          ...
          <Row style="padding: 15px">
            <div v-html="formModel.fields.content">{{formModel.fields.content}}</div>
          </Row>
        </fieldset>
...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.naturalResourcesUuid);
    },
  },
  methods: {
    ...
    handleRemove2(file) {
      ...
      this.formModel.fields.photo = this.uploadList2
        .map((x) => x.fileName)
        .join(",");
      // const fileList = this.$refs.upload.fileList;
      // this.$refs.upload.fileList.splice(fileList.indexOf(file), 1);
    ...
    loadNaturalResourcesList() {
      getNaturalResourcesList(this.stores.naturalR.query).then((res) => {
    ...
    docreateNaturalResources() {
      createNaturalResources(this.formModel.fields).then((res) => {
        if (res.data.code === 200) {
    ...
    doEditPlan() {
      editNaturalResources(this.formModel.fields).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.handleCloseFormWindow();
          this.loadNaturalResourcesList();
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.naturalResourcesUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
      ...
      deleteNaturalResources(ids).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.loadNaturalResourcesList();
          this.formModel.selection = [];
    ...
    handleEdit(row) {
      ...
      this.doLoadPublicity(row.naturalResourcesUuid);
    },
    handleShow(row) {
    ...
    doLoadPublicity(guid) {
      loadNaturalResources({ guid: guid }).then((res) => {
        ...
        if (res.data.data.cover != null) {
          ...
          for (let i = 0; i < list.length; i++) {
            this.uploadList.push({
              url:
                config.baseUrl.dev +
                "UploadFiles/Guide/NaturalR/" +
                list[i],
              status: "finished",
              name: "UploadFiles/Guide/NaturalR/" + list[i],
              fileName: list[i],
            });
          }
        }
