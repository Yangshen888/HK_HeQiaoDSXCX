  ...
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="名称" prop="foodName">
              <Input
          ...
          <Row style="padding: 15px">
            <div class="demo-upload-list" v-for="item in coveruploadList">
              <template v-if="item.status === 'finished'">
                ...
                <img v-if="item.url.substring(item.url.lastIndexOf('.'))!='.mp4'" :src="item.url" />
                <video v-if="item.url.substring(item.url.lastIndexOf('.'))=='.mp4'" :src="item.url"></video>
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="类型" prop="foodorTechan" label-position="right">
              <Select
                v-model="formModel.fields.foodorTechan"
                placeholder="类型"
                @on-change="xz"
              >
                <Option
          ...
          <Col span="12">
            <FormItem label="美食分类" prop="ytype" v-if="foodStaues">
              <RadioGroup v-model="formModel.fields.ytype">
                <Radio label="特色小吃"></Radio>
                <Radio label="全鱼宴"></Radio>
              </RadioGroup>
            </FormItem>
          </Col>
        </Row>
    ...
    <Modal title="查看图片" v-model="visible">
      <img v-if="imgName.substring(imgName.lastIndexOf('.'))!='.mp4'&&visible" :src="imgName"  style="width: 100%" />
      <video v-if="imgName.substring(imgName.lastIndexOf('.'))=='.mp4'&&visible" :src="imgName" style="width: 100%"></video>
...
import {
  ...
  batchCommand,
  deleteFood,
} from "@/api/intelligentguide/food";
...
export default {
      ...
      stores: {
        food: {
          ...
          columns: [
            ...
            { title: "名称", key: "foodName", tooltip: true, ellipsis: true },
            { title: "店铺名称", key: "title", tooltip: true, ellipsis: true },
            { title: "地址", key: "address", tooltip: true, ellipsis: true },
            { title: "价格", key: "price", tooltip: true, ellipsis: true },
            { title: "类型", key: "playType", tooltip: true, ellipsis: true },
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.foodUuid);
    },
  },
  methods: {
    ...
    coverhandleFormatError(file) {
      this.$Notice.warning({
        ...
        desc: "文件: " + file.name + " 不是png,jpg,jpeg",
      });
    },
    ...
    handleFormatError(file) {
      this.$Notice.warning({
        title: "The file format is incorrect",
        desc: "文件: " + file.name + " 不是png,jpg,mp4",
      });
    },
    handleMaxSize(file) {
      this.$Notice.warning({
        title: "Exceeding file size limit",
        desc: "文件 " + file.name + " 太大,超过30M.",
      });
    },
    handleBeforeUpload() {
    ...
    handleSubmitPlan() {
      ...
      if (valid) {
        this.formModel.fields.ytype =
          this.formModel.fields.ytype == "特色小吃" ? "0" : "1";
        ...
        let mp4list=this.uploadList.filter(x=>x.fileName.substring(x.fileName.lastIndexOf('.'))=='.mp4')
        console.log(mp4list)
        let pnum=this.uploadList.length-mp4list.length;
    ...
    xz(e) {
      ...
      console.log(e);
      if (e == 1) {
        this.foodStaues = false;
      } else {
        this.foodStaues = true;
      }
    },
    doLoadFood(guid) {
      loadFood({ guid: guid }).then((res) => {
        ...
        this.formModel.fields.ytype =
          this.formModel.fields.ytype == "1" ? "全鱼宴" : "特色小吃";
        this.formModel.fields.playType =
          this.formModel.fields.playType == "1" ? "古镇游" : "周边游";
        console.log(this.formModel.fields);
        if (res.data.data.foodorTechan == 1) {
          this.foodStaues = false;
        } else {
          this.foodStaues = true;
        }
        if (res.data.data.cover != null && res.data.data.cover != "") {
          ...
          for (let i = 0; i < list.length; i++) {
            this.coveruploadList.push({
              url: config.baseUrl.dev + "UploadFiles/Food/" + list[i],
              status: "finished",
              name: "UploadFiles/Food/" + list[i],
              fileName: list[i],
            });
          }
        }
