...
import {
  ...
  batchCommand,
  deleteSpecialPick,
} from "@/api/intelligentguide/specialpick";
...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.specialPickUuid);
    },
  },
  methods: {
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.specialPickUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
    ...
    handleEdit(row) {
      ...
      this.doLoadSpecialPick(row.specialPickUuid);
    },
    handleShow(row) {
      this.handleResetFormRole();
