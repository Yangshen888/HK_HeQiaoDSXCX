<template>
  <div>
    <Card>
      <dz-table
        :totalCount="stores.carpark.query.totalCount"
        :pageSize="stores.carpark.query.pageSize"
        :currentPage="stores.carpark.query.currentPage"
      ...
      >
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.carpark.query.kw"
                      placeholder="输入名称搜索..."
                      @on-search="handleSearchCarPark()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.carpark.query.isDeleted"
                        @on-change="handleSearchCarPark"
                        placeholder="删除状态"
                        style="width: 60px"
                      >
                        <Option
                          v-for="item in stores.carpark.sources
                            .isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                          >{{ item.text }}</Option
                      ...
                      <Select
                        slot="prepend"
                        v-model="stores.carpark.query.state"
                        @on-change="handleSearchCarPark"
                        placeholder="发布状态"
                        style="width: 60px"
                      >
                        <Option
                          v-for="item in stores.carpark.sources.stateSources"
                          :value="item.value"
                          :key="item.value"
                          >{{ item.text }}</Option
                        >
        ...
        <Table
          ...
          size="small"
          :highlight-row="true"
          :data="stores.carpark.data"
          :columns="stores.carpark.columns"
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="总车位" prop="truckSpace">
              <Input
                v-model="formModel.fields.truckSpace"
                placeholder="请输入总车位"
                style="width: 400px"
              />
            </FormItem>
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="收费说明">
              <Input
                v-model="formModel.fields.chargesNotes"
                placeholder="请输入收费说明"
                type="textarea"
                style="width: 400px"
              />
        ...
        <fieldset>
          <legend class="legend">图片(推荐大小370*330)</legend>
...
import {
  ...
  batchCommand,
  deleteCarPark,
} from "@/api/intelligentguide/carpark";
...
export default {
    ...
    return {
      ...
      formModel: {
        ...
        rules: {
          ...
          //price: [{type: "string",required: true,message: "请输入价格",trigger:'blur'}],
          price: [{ type: "string", required: true, validator: validate }],
          createTime: [
            {
              required: true,
              message: "请选择时间",
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.carParkUuid);
    },
  },
  methods: {
    ...
    loadCarParkList() {
      getCarParkList(this.stores.carpark.query).then((res) => {
        this.stores.carpark.data = res.data.data;
        this.stores.carpark.query.totalCount = res.data.totalCount;
        //console.log(this.stores.educaplan.data);
      });
    },
    handleSearchCarPark() {
      this.stores.carpark.query.currentPage = 1;
      this.loadCarParkList();
    },
    handleRefresh() {
      this.stores.carpark.query.currentPage = 1;
      this.loadCarParkList();
    ...
    handleSubmitPlan() {
      ...
      if (this.formModel.fields.truckSpace != "") {
        let reg = /^\d+$/;
        if (!reg.test(this.formModel.fields.truckSpace)) {
          this.$Message.warning("总车位请输入整数!");
          return;
        }
      }
      if (valid) {
        if (this.formModel.mode === "create") {
          this.docreateCarPark();
        }
        if (this.formModel.mode === "edit") {
          this.doEditPlan();
        }
    ...
    docreateCarPark() {
      createCarPark(this.formModel.fields).then((res) => {
        if (res.data.code === 200) {
    ...
    doEditPlan() {
      editCarPark(this.formModel.fields).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.handleCloseFormWindow();
          this.loadCarParkList();
    ...
    handleEdit(row) {
      ...
      this.doLoadCarPark(row.carParkUuid);
    },
    handleShow(row) {
    ...
    doLoadCarPark(guid) {
      loadCarPark({ guid: guid }).then((res) => {
        this.formModel.fields = res.data.data;
        if (res.data.data.picture != null && res.data.data.picture != "") {
          let list = res.data.data.picture.split(",");
          for (let i = 0; i < list.length; i++) {
            this.uploadList.push({
              url: config.baseUrl.dev + "UploadFiles/CarPark/" + list[i],
              status: "finished",
              name: "UploadFiles/CarPark/" + list[i],
              fileName: list[i],
            });
          }
        }
    ...
    handlePageChanged(page) {
      this.stores.carpark.query.currentPage = page;
      this.loadCarParkList();
    },
    handlePageSizeChanged(pageSize) {
      this.stores.carpark.query.pageSize = pageSize;
      this.loadCarParkList();
    },
  },
  mounted() {
